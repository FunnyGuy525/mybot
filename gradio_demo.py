...
parser.add_argument(  # 添加命令行参数
    ...
    type=str,
    required=True,
    help='Base model path')  # base_model参数，必须提供，用于指定模型路径
parser.add_argument('--lora_model', default=None, type=str,
                    help="If None, perform inference on the base model")  # lora_model参数，默认为None
parser.add_argument(
    '--tokenizer_path',
    default=None,
    type=str,
    help='If None, lora model path or base model path will be used')  # tokenizer_path参数，默认为None
parser.add_argument(
    '--gpus',
    default="0",
    type=str,
    help='If None, cuda:0 will be used. Inference using multi-cards: --gpus=0,1,... ')  # gpus参数，默认为"0"
parser.add_argument('--share', default=True, help='Share gradio domain name')  # share参数，默认为True
parser.add_argument('--port', default=19324, type=int, help='Port of gradio demo')  # port参数，默认为19324
parser.add_argument(
    '--max_memory',
    default=256,
    type=int,
    help='Maximum input prompt length, if exceeded model will receive prompt[-max_memory:]')  # max_memory参数，默认为256
parser.add_argument(
    '--load_in_8bit',
    action='store_true',
    help='Use 8 bit quantified model')  # load_in_8bit参数，默认为False
parser.add_argument(
    '--only_cpu',
    action='store_true',
    help='Only use CPU for inference')  # only_cpu参数，默认为False
parser.add_argument(
    '--alpha',
    type=str,
    default="1.0",
    help="The scaling factor of NTK method, can be a float or 'auto'. ")  # alpha参数，默认为"1.0"
args = parser.parse_args()  # 解析命令行参数并存储在args中
if args.only_cpu is True:  # 如果only_cpu参数为True，则gpus参数置为空字符串
    args.gpus = ""


# Set CUDA devices if available
os.environ["CUDA_VISIBLE_DEVICES"] = args.gpus  # 设置CUDA可见的设备


# Peft library can only import after setting CUDA devices
from peft import PeftModel  # 导入peft模块


# Set up the required components: model and tokenizer

def setup():
    global tokenizer, model, device, share, port, max_memory  # 声明全局变量
    max_memory = args.max_memory  # 将max_memory设置为命令行参数指定的值
    port = args.port  # 将port设置为命令行参数指定的值
    share = args.share  # 将share设置为命令行参数指定的值
    load_in_8bit = args.load_in_8bit  # 将load_in_8bit设置为命令行参数指定的值
    load_type = torch.float16  # 设置load_type为torch.float16
    if torch.cuda.is_available():  # 判断CUDA是否可用
        device = torch.device(0)  # 如果可用，将device设置为第一个GPU设备
    else:
        device = torch.device('cpu')  # 如果不可用，将device设置为CPU设备
    if args.tokenizer_path is None:  # 如果tokenizer_path参数为None，则将其设置为lora_model参数或base_model参数的值
        args.tokenizer_path = args.lora_model
        if args.lora_model is None:
            args.tokenizer_path = args.base_model
    tokenizer = LlamaTokenizer.from_pretrained(args.tokenizer_path)  # 使用指定路径的tokenizer进行初始化
    ...
    model_vocab_size = base_model.get_input_embeddings().weight.size(0)# 获取base_model的词汇表大小
